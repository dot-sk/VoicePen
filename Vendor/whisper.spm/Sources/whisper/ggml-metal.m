...
static NSBundle * ggml_metal_bundle_for_resources(void) {
    ...
    for (NSURL * candidateURL in candidateURLs) {
        ...
        for (NSString * bundleName in bundleNames) {
            NSURL * bundleURL = [candidateURL URLByAppendingPathComponent:[NSString stringWithFormat:@"%@.bundle", bundleName]];
...
static struct ggml_metal_context * ggml_metal_init(int n_cb) {
    ...
    // - if that fails, return NULL
    {
        NSBundle * bundle = nil;
#ifdef SWIFT_PACKAGE
        bundle = ggml_metal_bundle_for_resources();
#else
