...
tests:
  ...
  - VoicePenTests/History/VoiceHistoryEntryAgeGroupsTests.swift
...
- When the History UI has entries older than 7 days, VoicePen shall separate them into an Older group collapsed behind a disclosure while keeping entries from the last 7 days visible as recent sessions.
...
| Older history | History contains entries older than 7 days | Older entries are grouped under a collapsed Older disclosure |
...
- Automated: `VoicePenTests/App/VoicePenAppCommandTests.swift` covers main window sidebar ordering, General usage total caption, one-value disk usage display, and history row context/accessibility actions.
...
- Automated: `VoicePenTests/History/VoiceHistoryEntryAgeGroupsTests.swift` covers History grouping recent and older entries using the 7-day boundary.
...
- Manual: open History with entries older than 7 days and verify older sessions are collapsed under Older while recent sessions remain visible.
