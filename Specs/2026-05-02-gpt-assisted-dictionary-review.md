...
- When VoicePen saves a completed transcription history entry, it shall store the selected model metadata and VoicePen app version used for that transcription.
...
- When building the prompt, VoicePen shall not include model identifiers, backend names, model versions, VoicePen app versions, or model performance statistics.
...
- When the user copies a dictionary review prompt, VoicePen shall show temporary copied feedback on the copy action.
...
- When the user imports dictionary CSV from the clipboard and the clipboard is empty or does not contain valid dictionary CSV, VoicePen shall not open the import preview and shall leave any existing import preview state closed.
- When the dictionary import preview is open, Escape shall close the preview without changing the dictionary.
...
| Copy prompt feedback | User copies a dictionary review prompt | The copy action temporarily shows copied feedback |
...
| History metadata | Stored history contains ids, timestamps, model metadata, and app version metadata | Prompt does not include ids, timestamps, model fields, app versions, or model stats |
...
| Empty clipboard import | Clipboard has no text | Import preview does not open and dictionary remains unchanged |
| Escape preview | Import preview is open | Escape returns to the dictionary without importing |

## Test Mapping

- Automated: `VoicePenTests/History/VoiceHistoryStoreTests.swift` covers saving, loading, and `Unknown` export fallback for transcription model metadata and app version metadata on history entries.
...
- Automated: `VoicePenTests/App/AppControllerTests.swift` covers copying review prompts, preparing clipboard import previews without mutating the dictionary, rejecting empty or invalid clipboard imports before preview, confirming previews through the shared dictionary import path, and rejecting stale previews after dictionary changes.
- Automated: `VoicePenTests/App/VoicePenAppCommandTests.swift` covers that the dictionary import preview exposes Escape as a cancel path.
...
Store enough metadata for local history and diagnostics without depending on a future manifest lookup: model id, display name, backend or source kind, model version, and the VoicePen app version used for decoding. Adding this storage must be backward-compatible, and dictionary review prompts must not export those model or app version fields.
