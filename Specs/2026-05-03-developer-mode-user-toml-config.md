...
- When the user chooses to reload config from the Settings screen, VoicePen shall show short success feedback on the reload control without resizing the control.
- When the user switches to the Settings screen, VoicePen shall refresh TOML-backed settings after the settings view update rather than synchronously publishing during SwiftUI view construction.
